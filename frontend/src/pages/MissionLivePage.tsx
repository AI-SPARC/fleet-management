...
import type { MissionTrajectoryPoint, RobotState } from '../api/client';
import { MapGraph } from '../components/map/MapGraph';
import { Badge } from '../components/ui/badge';
import { Button } from '../components/ui/button';
import { Card, CardContent, CardHeader, CardTitle } from '../components/ui/card';
import { useMap } from '../hooks/useMaps';
import { useMission, useMissionRoute, useMissionTrajectory } from '../hooks/useMissions';
...
export function MissionLivePage() {
  ...
  const trajectory = useMissionTrajectory(missionId);
  ...
  const now = useNow();
  const replayPoint = replayIndex === null ? undefined : trajectory.data?.[replayIndex];
  const displayedState: RobotState | undefined = replayPoint
    ? {
        ...state.data,
        orderId: missionId,
        lastNodeId: replayPoint.lastNodeId,
        batteryCharge: replayPoint.batteryCharge,
        receivedAt: replayPoint.timestamp,
        agvPosition: {
          x: replayPoint.x,
          y: replayPoint.y,
          theta: replayPoint.theta,
          mapId: replayPoint.mapId,
        },
      }
    : state.data;
  const stateAge = telemetryAge(displayedState, now);
  const stale = stateAge === undefined || stateAge > STALE_AFTER_MS;
  const belongsToMission = displayedState?.orderId === missionId;
  const routeNodes = route.data?.nodeKeys ?? [];
  const routeIndex = displayedState?.lastNodeId
    ? routeNodes.indexOf(displayedState.lastNodeId)
    : -1;
  ...
  return (
    <div className="grid gap-7">
      ...
      <section className="grid grid-cols-4 gap-3">
        ...
        <Metric
          label="Telemetry"
          tone={replayPoint ? 'normal' : stale ? 'danger' : 'normal'}
          value={
            replayPoint
              ? 'Replay'
              : stateAge === undefined
                ? 'No data'
                : stale
                  ? `Stale · ${formatAge(stateAge)}`
                  : 'Live'
          }
        />
        <Metric
          label="Battery"
          value={displayedState?.batteryCharge == null ? '—' : `${displayedState.batteryCharge.toFixed(1)}%`}
        />
      </section>

      {!belongsToMission && displayedState && (
        <div className="rounded-xl border border-amber-300 bg-amber-50 p-3 text-sm text-amber-900">
          Latest robot state belongs to order {displayedState.orderId ?? 'none'}, not this mission.
        </div>
      )}
      ...
      {map.data ? (
        <MapGraph
          ...
          map={map.data}
          robots={robot.data && displayedState ? [{ robot: robot.data, state: displayedState }] : []}
        />
      ) : (
      ...
      <section className="grid grid-cols-3 gap-4">
        ...
        <DetailCard title="Robot state">
          <p className="m-0 text-sm">Last node: {displayedState?.lastNodeId ?? '—'}</p>
          <p className="mb-0 mt-2 text-xs text-muted-foreground">
            Mode: {displayedState?.operatingMode ?? 'unknown'} · Order: {displayedState?.orderId ?? 'none'}
          </p>
        </DetailCard>
        <DetailCard title="Safety & errors">
          <p className="m-0 text-sm">{displayedState?.errors?.length ?? 0} active errors</p>
          <p className="mb-0 mt-2 text-xs text-muted-foreground">
            {safetyLabel(displayedState)}
          </p>
        </DetailCard>
      </section>
    </div>
...
function ReplayControls({
  ...
  points: MissionTrajectoryPoint[];
  index: number | null;
  onChange: (index: number | null) => void;
}) {
  const currentIndex = index ?? Math.max(points.length - 1, 0);
  const current = index === null ? undefined : points[currentIndex];
  return (
    <Card className="shadow-none">
      <CardContent className="grid gap-3 p-4">
        <div className="flex items-center justify-between gap-4">
          <div>
            <strong className="text-sm">Trajectory replay</strong>
            <span className="ml-2 text-xs text-muted-foreground">
              {points.length} position samples
            </span>
          </div>
          <span className="text-xs text-muted-foreground">
            {current ? new Date(current.timestamp).toLocaleString() : 'Showing live state'}
          </span>
        </div>
        <input
        ...
        <div className="flex gap-2">
          ...
          <Button
            disabled={index === null || currentIndex === 0}
          ...
          <Button
            disabled={index === null || currentIndex >= points.length - 1}
            onClick={() => onChange(Math.min(currentIndex + 1, points.length - 1))}
            size="sm"
            variant="outline"
          >
            Next
          </Button>
          <Button disabled={index === null} onClick={() => onChange(null)} size="sm">
            Return to live
          </Button>
        </div>
      </CardContent>
